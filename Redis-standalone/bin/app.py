...
class App:
    ...
    def __reconfigure_conf(self, conf_file_path, redis_conf=True):
        self.logger.info("Reconfigure %s with master [%s, %s]" % (
            conf_file_path, self.config.get_master_ip(), self.config.get_master_port()))
    ...
    def __single_node_start(self, from_who=None):
        ...
        if not self.__check_process_alive('redis-mon', use_ps=True):
            start_redis_mon_cmd = "systemctl enable redis-mon;systemctl start redis-mon"
            self.__exec_cmd(start_redis_mon_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def start(self):
        ...
        if not self.config.will_be_deleted() and self.config.is_master(exclude=False):
            ...
            redis_mon_cmd = "systemctl enable redis-mon;systemctl start redis-mon"
            process_redis_mon = self.__exec_cmd(redis_mon_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            self.logger.warn("start slave")
            if process_redis.returncode == 0 and process_redis_mon.returncode == 0:
                self.__apply_new_config()
    ...
    def __stop(self):
        ...
        if self.__check_process_alive('redis-mon', use_ps=True):
            stop_redis_mon_cmd = Constants.REDIS_HOME + "/bin/stop-redis-mon.sh"
            self.__exec_cmd(stop_redis_mon_cmd)
        stop_server_cmd = Constants.REDIS_HOME + "/bin/stop-redis-server.sh"
        self.__exec_cmd(stop_server_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def check(self):
        ...
        if not self.__check_process_alive('redis-mon'):
            return False
        return True
    ...
    def __apply_new_config(self):
        ...
        if self.__check_process_alive("redis-server") and len(update_config) > 0:
            ...
            for key, value in update_config.iteritems():
                if self.config.is_requirepass():
                    cmd = "%s/bin/redis-cli -a %s %s set %s %s" % (
                        Constants.REDIS_HOME, self.config.get_password(), config_command, key, value)
                else:
